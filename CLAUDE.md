...
모든 세션 시작 시 Claude는 다음을 수행:

1. 루트 `CLAUDE.md` (이 파일) 전체 로드
2. `docs/current/` 하위 6개 문서 전체 로드 (reference-ids 포함)
3. 현재 작업 디렉토리 / 대상 프로젝트 파악
4. `.claude/devlog-cache.json` 확인 후 필요 시 Notion MCP로 최근 5건 갱신
   - 캐시 TTL 24시간. 신선하면 재조회 생략, 만료 시만 MCP 호출
   - 갱신 후 `.claude/devlog-cache.json`에 저장 (entries 배열 + `fetched_at` ISO)
5. 캐시의 `회고.다음개선` 요약 → "오늘 특히 주의할 점" 3줄 이내 제시
6. **`.claude/pending-devlog.json` 존재 시 반드시 첫 액션으로 Notion 등록** (§1-6 자동화 참조)
   - `requires_registration: true`면 사용자 다른 요청 전에 등록 의무
   - `requires_registration: false`면 CEO 명시 요청 시만 등록, 아니면 파일 삭제 후 진행

**훅 지원**: `hooks/session-start-memory.mjs`가 단계 4·6에 필요한 파일 존재·신선도를 검사해 stderr로 안내한다. 실제 MCP 호출·3줄 작성은 Claude가 수행 (토큰은 훅 프로세스에 노출되지 않음).
...
세션 종료 시 Claude는 Notion MCP로 💻 개발 노트 DB에 v2.0 엔트리 1건을 등록한다.

- 하네스 자체 작업(δ 패턴): 반드시 기록 (`requires_registration: true`)
- 프로젝트 작업(α·β·γ 패턴): 반드시 기록 + 해당 프로젝트의 파일 로그에도 요약
- 10분 이내 소소한 탐색 세션: `requires_registration: false`로 자동 표시. CEO 명시 요청 시만 등록

**훅 자동화**: `hooks/on-stop.mjs`가 세션 종료 시 `.claude/pending-devlog.json`을 자동 작성한다. 이 파일은 다음 세션 시작 시 `session-start-memory.mjs`가 감지해 Claude에게 등록 의무를 알리고, Claude가 MCP로 실제 등록을 수행한 뒤 파일을 삭제한다. 훅은 MCP를 직접 호출하지 않는다 (토큰 노출·timeout 회피).

payload 스키마: `version`·`session`(started/ended/elapsed_minutes)·`task`·`hints`(pattern/project/category/result candidate)·`evidence`(commits/files_changed/exit_status)·`requires_registration`. 상세는 `hooks/on-stop.mjs` 주석 참조.
...
`hooks/` 디렉토리. Lifecycle 매핑은 `settings.json`에 정의, `scripts/install-hooks.mjs` 실행으로 `~/.claude/settings.json`에 머지되어 활성화.

| 파일 | 라이프사이클 | 역할 |
|---|---|---|
| `_auto-heal.mjs` | SessionStart (inline) | 훅 자가치유 체크 |
| `session-start-memory.mjs` | SessionStart | evolving-rules + memory 로딩 |
| `harness-drift-check.mjs` | SessionStart | repo↔user scope drift 감지 |
| `on-prompt-unified.mjs` | UserPromptSubmit | CLI 체크 + 품질 GC + 위험 감지 통합 |
| `enforce-delegation.mjs` | PreToolUse (Bash\|WebSearch\|WebFetch\|Write\|Edit) | v6.0 위임 규칙 강제 |
| `pre-secret-scan.mjs` | PreToolUse (Write\|Edit) | 시크릿 패턴 사전 스캔 |
| `acl-post-build.mjs` | PostToolUse (Bash) | 빌드 결과 검증 |
| `acl-subagent-stop.mjs` | SubagentStop | 타입 체크 + 빌드 |
| `on-stop.mjs` | Stop | 활동 로그 + 인코딩 게이트 |
| `evolving-guardrails.mjs` | Stop | 반복 에러 진화 규칙 업데이트 |

**활성화**:
```
node scripts/install-hooks.mjs --dry-run   # 변경 미리보기
node scripts/install-hooks.mjs             # 실제 머지 (백업 자동 생성)
```

머지는 `~/.claude/settings.json`의 `hooks` 필드만 교체하고 `mcpServers`·`permissions`는 보존한다.
